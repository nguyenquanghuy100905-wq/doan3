...
exports.getDetailnewsByNewsId = async (req, res) =>{
	try{
		const detailnews = await Detailnews.getDetailnewsByNewsId(req.query.news_id);
		res.status(200).send(detailnews);
	} catch (error) {
		res.status(500).send({ message: error.message });
	};
...
exports.createDetailnews = async (req, res) => {
	try {
		...
		const detailnewsId = await Detailnews.createDetailnews( news_id, content );
...
exports.updateDetailnews = async (req, res) => {
	try {
		...
		const updatedDetailnews = await Detailnews.updateDetailnews(id, news_id, content );
